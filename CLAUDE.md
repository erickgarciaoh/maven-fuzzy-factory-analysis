...
- Brief de delivery (Fase 5, aprobado 2026-07-03): [`docs/05_visualization_brief.md`](docs/05_visualization_brief.md) + [`PRODUCT.md`](PRODUCT.md) (register brand) + [`DESIGN.md`](DESIGN.md) (tokens de marca fijados para el build). Decisiones acordadas: narrativa **scrollytelling guiado** (realce progresivo sobre charts ya renderizados, NO content-gating; reduced-motion → long-scroll plano) · tema **light + toggle dark** · interactividad **selectiva** (controles solo en el funnel: device + trimestre; hover en el resto; sin filtros globales). Arco: Hero (CVR 4,14%→8,44%, +104%) → 01 Funnel → 02 A/B (incluye el test perdedor lander-4, no se esconde) → 03 Descomposición (waterfall) → 04 Economía → Coda (límites + provenance SQL). Un chart primario por acto, ámbar reservado al dato focal. Fase 6 arranca con `/impeccable craft` sobre el brief.
...
- Capa de consumo (Fase 4): esquema `analysis` (`src/07_create_analysis_views.sql`), 4 vistas, cada una exportada a `docs/data/*.json`:
  - `vw_funnel_conversion` (P4): 7 pasos (landing→products→product detail→cart→shipping→billing→thank-you) por trimestre y device. Confirma hipótesis mobile: Q4-2014 desktop 9,6% end-to-end vs mobile 3,4%.
  - `vw_ab_test_results` (P3): 4 tests reconstruidos por ventana de solapamiento real (perfilada, no asumida) — home vs lander-1 (2012-06-19→07-29, +0,81pp, no significativo), lander-2 vs lander-4 desktop (2014-02-02→04-19, **lander-4 perdió** -1,47pp, z=-3,23), lander-2 vs lander-5 desktop (2014-08-02→10-31, +1,82pp, z=4,55), billing vs billing-2 (2012-09-10→2013-01-05, +17,0pp, z=9,84). Todas las demás transiciones de lander fueron cutover instantáneo (sin control concurrente) → excluidas del test z. Z-test de proporciones con varianza agrupada, umbral |z|≥1,96/2,576; lift anualizado extrapola la tasa diaria de la propia ventana de test (no estacional).
  - `vw_cvr_decomposition` (P1): descomposición aditiva simétrica (Oaxaca) por `channel_group`, 2012 vs 2015. CVR 4,14%→8,44% (cuadra con Fase 2); +4,27pp por mejora intra-canal (gsearch solo aporta +3,26pp) vs +0,03pp por mix-shift (casi neutro).
  - `vw_channel_economics` (P2): grain `channel` (sesión→orden→revenue→cogs→margen→refund por canal/trimestre) + grain `product` (refund por producto/trimestre, cuadra con Fase 2: Mr. Fuzzy 1.237 refunds/$61.837,63, Sugar Panda 6,04% tasa). Sin CAC/ROAS (dato no disponible); margen bruto only.
  - Exportación JSON vía `sqlcmd -C` (certificado no confiable en esta instancia, requiere `-C`) + `FOR JSON PATH`; sqlcmd envuelve el output nvarchar(max) en líneas de 2033 chars sin delimitador — hay que concatenar antes de parsear (`replace(/[\r\n]+/g,'')`), o preferir MCP `execute_select` para result sets que quepan en max_rows.
- Presentación (Fase 6): data-story de una página en `docs/index.html` + `docs/assets/{styles.css,app.js}`, estático (ECharts CDN, sin build). `app.js` fetchea los 4 JSON y **calcula todos los números en runtime** (KPIs del hero se derivan sumando el grain `channel` de economics: 472.871 · 32.313 · $1.94M · $60 AOV · 62,7% margen; hero CVR 4,14%→8,44% de la fila TOTAL de decomposition). Tokens de marca copiados a `docs/assets/tokens.css`+`tokens.dark.css` (autocontenido; no enlazar fuera del repo). 6 charts temados leyendo tokens vía `getComputedStyle`; re-render en toggle light/dark. **Contraste (decisión clave):** el ámbar de marca `#C27C35` da 2,68:1 sobre canvas → falla AA en texto; separados dos usos: rellenos de chart en ámbar brillante (color de serie), pero texto/ícono/foco ámbar usan `--amber-text #8F5723` (4,69:1) y el delta verde `--green-text #18703B` (4,88:1). Todo el texto pasa AA en ambos temas (verificado sobre color renderizado). Scrollytelling = realce sobre charts ya renderizados (IntersectionObserver); reduced-motion → estado final directo. Fix responsive: `min-width:0` en hijos de grid para que el `<pre>` SQL no rompa el ancho. Links de la coda → `github.com/erickgarciaoh/maven-fuzzy-factory-analysis` (`/blob/HEAD/` branch-agnóstico); repo se crea recién en Fase 8. Auditada y refinada en navegador (Playwright): light/dark, 360/768/1360, reduced-motion, 0 errores de consola.
